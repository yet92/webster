...
const IconWidget: React.FC = () => {
	...
	const icons = useMemo(() => {
		...
		return iconList.filter(
			(_icon) => _icon.name.indexOf(searchKeyword) !== -1
		);
	}, [searchKeyword]);
		...
		<Col className={[sizeStyles['mx-h-30vh']].join(' ')}>
			<Form>
				<Form.Group className='mb-3' controlId='iconKeyword'>
					...
					<Form.Control
						className='tw-border-contrast  tw-bg-secondary tw-p-2'
...
const IconThumbnail: React.FC<{
	...
	<Figure
		as={Col}
		className={[alignStyles.absoluteCenter, alignStyles.wrapTrue].join(' ') + "tw-text-text"}
	>
		<Drag
			dragType='copyMove'
			dragSrc={{
