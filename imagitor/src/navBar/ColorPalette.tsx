...
export default function ColorPalette({ selectedItems }: { selectedItems: any }) {
  ...
  const [color, setColor] = useState<ColorResult>("#ffffff" as unknown as ColorResult);
  ...
  const { currentColors } = useSelector((selector: StoreState) => selector.toolSelection);
  ...
  const onAddColor = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (selectedItems[0]) selectedItems[0].getStage()?.batchDraw();
      ...
      <Dropdown.Menu className="tw-w-[10vw] tw-bg-secondary tw-text-text">
        <div className="p-2 tw-flex tw-w-full tw-flex-col tw-justify-center tw-gap-2">
          ...
          >
            ...
            <Modal onClose={onClose} onHide={onClose} className="tw-text-text" centered show={show}>
              <Modal.Header className="tw-bg-secondary" closeButton>
                <Modal.Title id="contained-modal-title-vcenter">Pick Color</Modal.Title>
              </Modal.Header>
              <Modal.Body className="tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-5 tw-bg-secondary">
                ...
                />
                <div onClick={onAddColor} className="tw-w-2/3 tw-rounded-xl tw-bg-contrast tw-p-3 tw-text-center">
                  Add Color
                </div>
              </Modal.Body>
            </Modal>
          ...
          <div className="tw-flex tw-w-full tw-flex-wrap tw-items-center tw-justify-evenly tw-gap-2">
            {currentColors.map((clr) => (
              ...
              >
                <div className="tw-h-[30px] tw-w-[30px]" style={{ backgroundColor: clr }}></div>
                <span className="tw-text-xs tw-text-contrast">{clr}</span>
              </div>
            ))}
          </div>
        </div>
...
const CustomToggle = React.forwardRef<HTMLButtonElement>(
  ...
  function CustomToggleItem({ children, onClick, data }, ref) {
    const { toolColor, currentColors } = useSelector((selector: StoreState) => selector.toolSelection);
    return (
      <OverlayTrigger placement="right" overlay={<Tooltip>Pick Color</Tooltip>}>
        <button
          style={{ backgroundColor: toolColor }}
          className={`text-text ${
            currentColors.length ? "hover:tw-bg-contrast/10" : "tw-bg-secondary hover:tw-bg-contrast"
          } tw-h-full tw-w-full tw-rounded-md tw-border-contrast/20 tw-p-1 tw-px-3 `}
          ref={ref}
          onClick={(e) => {
            e.preventDefault();
            onClick(e);
        ...
        >
          <i className={`bi-palette tw-text-text`}>{children}</i>
        </button>
      </OverlayTrigger>
    );
  }
